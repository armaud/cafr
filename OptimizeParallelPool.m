                    ...
                    if i~=j
                        ...
                        parfor k = 1:l
                          ...
                          %val=[val abs([corr2(I(:,:,k,j),I(:,:,k,i))]) abs([corr2(I(:,:,k,abs(f-j)),I(:,:,k,abs(f-i)))]) abs([corr2(I(:,:,k,abs(f-j)),I(:,:,k,abs(i)))]) abs([corr2(I(:,:,k,abs(j)),I(:,:,k,abs(f-i)))])];
                            val = max([ abs([corr2(I(:,:,k,j),I(:,:,k,i))]) abs([corr2(I(:,:,k,abs(f-j)),I(:,:,k,abs(f-i)))]) abs([corr2(I(:,:,k,abs(f-j)),I(:,:,k,abs(i)))]) abs([corr2(I(:,:,k,abs(j)),I(:,:,k,abs(f-i)))])]);
                            val1=val
                        end
                       [row column]=find(array1==bin2dec(dec2bin(max(val1)*1000,10))/1000);
...
threshold =threshold + array1(find(diffMat(i,:)==result(i)));
end

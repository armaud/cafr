function filts=optimizationAlgoAutoPartialFeatures(threshold,stepSize,offsetinit)
% threshold = 0.5
% stepSize = 0.05
...
performStep = 5;
while(threshold <= 1)
for recursiveIterations = 1:performStep
    Ca=[];
    layerDiffCounter = -1;
for ii=1:length(net.Layers)
    ...
    if length(layer)>3
        ...
        ii;
        offset = offsetinit;
        for numf = 1:offset:sum(net.Layers(ii).NumFilters)
%             [numf ii]
            if numf+offset>sum(net.Layers(ii).NumFilters)
                offset = sum(net.Layers(ii).NumFilters)-numf;
            end
        ...
        disp(['Loading Feature Maps from: '  layer ' from feature map from: ' num2str(numf) ' - ' num2str(numf+offset) '.'])
        I = deepDreamImage(net,layer,numf:numf+offset, ...
        'PyramidLevels',1, ...
        'Verbose',0,'ExecutionEnvironment','gpu');
        [r c l f]=size(I);
        ...
        val=[];
            for j=1:f
                for i = 1:f
                    ...
                    Ca=[Ca ii];% i+numf j+numf];% abs(max(val))]
                    end
                    val=[];
                    end
%                  [ i, j ]   

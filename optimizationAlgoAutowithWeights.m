function filts=optimizationAlgoAutowithWeights(threshold,stepSize,depth)
% threshold = 0.5
% stepSize = 0.05
...
%         disp('Computing on GPU...!')
% else
    disp('Computing on CPU...!')
% end
while(threshold <= 1)
    perfromSteps = 3;
for recursiveIterations = 1:perfromSteps
    Ca=[];
    disp(['Performing Step '  num2str(recursiveIterations) ' / ' num2str(perfromSteps)])
    layerDiffCounter = -1;
...
%         if ~isempty(gpuDevice(1))
%         I = gpuArray(net.Layers(ii).Weights);
%         else
